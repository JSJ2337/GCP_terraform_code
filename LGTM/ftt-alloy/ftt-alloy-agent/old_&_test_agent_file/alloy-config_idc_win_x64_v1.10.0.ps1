...
$INSTANCE_TYPE  = "windows-server-x86"
...
prometheus.exporter.windows "win_metrics" {
  enabled_collectors = ["cpu", "cs", "logical_disk", "net", "os", "system", "time", "diskdrive"]
}
...
# ========== 레지스트리 설정 수정 (올바른 config 경로) ==========
Write-Host "📝 레지스트리 설정 수정 중..."
...
if (Test-Path $regPath) {
    ...
    Set-ItemProperty -Path $regPath -Name "Arguments" -Value $newArgs
    Write-Host "✅ 레지스트리 Arguments 수정 완료"
    Write-Host "   Config 경로: $configPath"
}
