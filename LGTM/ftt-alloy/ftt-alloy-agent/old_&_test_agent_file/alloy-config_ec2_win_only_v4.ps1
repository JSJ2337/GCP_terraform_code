...
loki.process "enrich" {
  ...
  forward_to = [loki.write.default.receiver, loki.process.metrics_extract.receiver]
}

// Loki 로그에서 메트릭 추출
loki.process "metrics_extract" {
  stage.metrics {
    match_all = true
    metric "loki_log_errors_total" {
      match = "{level=\"error\"}"
      value = 1
      type  = "counter"
    }
    metric "loki_log_warnings_total" {
      match = "{level=\"warn\"}"
      value = 1
      type  = "counter"
    }
    metric "loki_log_infos_total" {
      match = "{level=\"info\"}"
      value = 1
      type  = "counter"
    }
    metric "loki_total_log_lines" {
      match = "{}"
      value = 1
      type  = "counter"
    }
...
prometheus.scrape "alloy_internal_metrics" {
  targets         = [{"__address__" = "localhost:9090", "job" = "alloy_internal_metrics"}]
  scrape_interval = "15s"
  forward_to      = [prometheus.remote_write.mimir.receiver]
}
