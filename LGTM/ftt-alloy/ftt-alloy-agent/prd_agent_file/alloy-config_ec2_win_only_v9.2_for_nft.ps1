...
//////////////////////////////////////////////////////
// Custom File Log collection - 수정된 버전
//////////////////////////////////////////////////////

// 파일 탐색 컴포넌트
local.file_match "pm2_logs" {
  path_targets = [{__path__ = "C:/Users/block-svcop/.pm2/logs/*.log"}]
  sync_period = "10s"  // 파일 변경 감지 주기
}

local.file_match "documents_logs" {
  path_targets = [{__path__ = "C:/Users/block-svcop/Documents/logs/*.log"}]
  sync_period = "10s"
}
...
loki.source.file "custom_logs" {
  targets = concat(
    local.file_match.pm2_logs.targets,
    local.file_match.documents_logs.targets,
  )
  tail_from_end = true  // 파일 끝부터 읽기 시작
