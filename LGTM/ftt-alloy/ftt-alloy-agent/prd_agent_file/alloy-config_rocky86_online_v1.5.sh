...
set -euo pipefail
##################################################################################
# install_alloy_online.sh - Rocky Linux 8.6 IDC Environment (Online DNF Installation)
##################################################################################
ACCOUNT_ID="idc-spd"
JOB_NAME="prod"
...
ALLOY_VERSION="1.10.0"                   # 설치할 Alloy 버전 (온라인 DNF)
...
if $INSTALL_ALLOY; then
  ...
  # Grafana DNF 리포지토리 추가
  echo "[INFO] Adding Grafana DNF repository..."
...
EOF
  ...
  # DNF를 사용하여 Alloy 설치
  echo "[INFO] Installing Alloy v${ALLOY_VERSION} using DNF..."
  sudo $PKG -y install "alloy-${ALLOY_VERSION}" || {
    echo "[ERROR] Alloy DNF installation failed."
    echo "Please check your internet connection and repository settings."
    exit 1
  }

  echo "[INFO] Alloy online installation complete."
fi
...
if command -v alloy >/dev/null 2>&1; then
  ...
  echo "[ERROR] Alloy installation verification failed."
  exit 1
fi
...
otelcol.receiver.otlp "lgtm_tempo_trace" {
...
if command -v alloy >/dev/null 2>&1; then
  alloy validate "$ALLOY_CONFIG_PATH" || {
    echo "[ERROR] Alloy configuration validation failed."
    exit 1
  }
  echo "[INFO] Alloy configuration validation complete."
else
  echo "[WARN] Alloy command not found, skipping configuration validation."
fi
...
echo -e "\n✅ Alloy + Loki + Mimir + Tempo + Pyroscope configuration complete (Rocky Linux 8.6 Online DNF)"
echo "   System   : ${INSTANCE_TYPE} (${HOSTNAME})"
echo "   Version  : Alloy ${ALLOY_VERSION} (Online DNF Installation)"
