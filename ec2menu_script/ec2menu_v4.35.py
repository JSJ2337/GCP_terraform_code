...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v4.35 (v3.2 Linux 접속 방식 적용)

- v4.34 버전을 기반으로, Linux SSM 접속 로직을 안정적인 v3.2 버전의 방식으로 교체.
- ssm_cmd, launch_linux_wt 함수를 v3.2 버전의 코드로 원복하여 접속 안정성을 확보.
- 새로고침, RDP 다중 접속, Jump Host 이름 표시 기능은 그대로 유지.
"""
import os
import sys
import argparse
...
def connect_to_rds(manager: AWSManager, tool_path: str, region: str):
    while True:
        dbs = manager.list_rds_endpoints(region)
        if not dbs:
            print("\n⚠ RDS 인스턴스가 없습니다")
            return
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.35')
