...
def ssm_cmd(profile, region, iid=None, doc='AWS-StartInteractiveCommand', params='{"command":["bash -l"]}', target=None):
    ...
    # Windows -> WSL 셸을 통과할 때 JSON이 깨지지 않도록 내부의 큰따옴표를 이스케이프 처리
    escaped_params = params.replace('"', '\"')
    cmd += ['--document-name', doc, '--parameters', escaped_params]
    return cmd
...
def ec2_menu(manager: AWSManager, region: str):
    ...
    print("\n--- [ EC2 Instances ] ---")
    # 헤더 출력
    print(f" {'No':<3} {'Name':<30} {'Instance ID':<22} {'Public IP':<18} {'Private IP':<18}")
    print("-" * 95)

    for idx, i_data in enumerate(insts, 1):
        i = i_data['raw']
        print(f" {idx:<3} {i_data['Name']:<30} {i['InstanceId']:<22} {i_data['PublicIp']:<18} {i_data['PrivateIp']:<18}")
    print("-" * 95)
