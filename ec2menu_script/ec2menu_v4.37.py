...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v4.37 (RDS, ElastiCache 접속 오류 수정)

- RDS 접속 시 `list_rds_endpoints` -> `get_rds_endpoints` 함수명 오류 수정.
- ElastiCache/RDS 접속 시 잘못된 `ssm_cmd` 함수 호출로 인한 `TypeError` 오류 수정.
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.37')
