...
def ec2_menu(manager: AWSManager, region: str):
    ...
    for idx, i in enumerate(insts, 1):
        ...
        print(f" {idx:2d}) {i['InstanceId']} | {platform:<15} | {az:<15} | {name}")
    print("-----------------------------------------------------------------------------------\n")

    sel = input("접속할 인스턴스 번호 입력 (b=뒤로): ").strip().lower()
    if not sel or sel == 'b':
        return
...
# Main 흐름
# ----------------------------------------------------------------------------
def main():    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.5')    parser.add_argument('-p', '--profile', help='AWS 프로파일 이름')    parser.add_argument('-d', '--debug', action='store_true', help='디버그 모드')    args = parser.parse_args()    setup_logger(args.debug)    profile = args.profile or choose_profile()    manager = AWSManager(profile)    while True:        region = choose_region(manager)        if not region:            break                print(f"\n--- [ Main Menu ] ---")        print(f"Profile: {profile} | Region: {region}")        print("---------------------")        print(" 1) EC2 인스턴스 연결")        print(" 2) RDS 데이터베이스 연결")        print(" 3) ElastiCache 클러스터 연결")        print("---------------------")        sel = input("선택 (b=프로파일/리전 재선택, Enter=종료): ").strip().lower()        if sel == '1':            ec2_menu(manager, region)        elif sel == '2':            connect_to_rds(manager, DEFAULT_DB_TOOL_PATH, region)        elif sel == '3':            connect_to_cache(manager, region)        elif sel == 'b':            continue        else:            break
