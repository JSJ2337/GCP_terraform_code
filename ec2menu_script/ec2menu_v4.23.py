...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v5.2 (새로고침, RDP 다중 접속 기능 추가)

- HeidiSQL의 안정적인 CLI를 사용하여 완전 자동화된 DB 접속 기능 구현
- 실행 환경(WSL/Windows)을 감지하여 도구 경로를 자동으로 변환
- EC2 인스턴스가 있는 리전만 선택하도록 필터링
- RDS/ElastiCache 접속 시 여러 SSM 관리 인스턴스 중 Jump Host를 선택하는 기능 추가
- 각 메뉴에 새로고침(r) 기능 추가
- Windows RDP 다중 접속 기능 추가 (쉼표로 구분)
"""
import os
import sys
import argparse
...
def choose_jump_host(ssm_targets: list):
    ...
    if len(ssm_targets) == 1:
        print(f"\n(info) 유일한 SSM 인스턴스 '{ssm_targets[0]}'를 Jump Host로 사용합니다.")
        return ssm_targets[0]
...
def launch_linux_wt(profile, region, iid):
    ...
    if wt:
        subprocess.Popen([wt,'new-tab','wsl.exe','--_wsl_user','root','-e',*cmd], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    else:
        subprocess.run(cmd)
        ...
        while True:
            ...
            for choice_idx in valid_choices:
                ...
                if inst.get('PlatformDetails', 'Linux').lower().startswith('windows'):
                    local_port = 10000 + (int(inst['InstanceId'][-3:], 16) % 1000) + (choice_idx - 1)
                    ...
                    print("(info) 새 터미널에서 SSM 세션이 시작되었습니다.")
            
            if any(insts[c - 1]['raw'].get('PlatformDetails', 'Linux').lower().startswith('windows') for c in valid_choices):
        ...
        try:
            ...
            if tool_path and Path(tool_path).exists():
                for i, choice_idx in enumerate(valid_choices):
                    ...
                    command = [
                        tool_path, f"-n={network_type}", f"-h=localhost",
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v5.2')
