...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v4.30 (접속 로직 수정)

- v4.22 원본 코드를 기준으로, 요청된 기능만 다시 구현하여 안정성을 확보.
- 각 메뉴에 목록 새로고침(r) 기능 추가.
- Windows RDP 다중 접속 기능 추가 (쉼표로 구분하여 입력).
- Linux 접속은 기존과 같이 단일 접속만 허용하도록 로직 수정.
"""
import os
import sys
import argparse
...
# ----------------------------------------------------------------------------
# EC2 메뉴 (Linux 단일 접속, Windows 다중 접속으로 수정)
# ----------------------------------------------------------------------------
        ...
        try:
            ...
            for i, choice_idx in enumerate(valid_choices):
                ...
                if inst.get('PlatformDetails', 'Linux').lower().startswith('windows'):
                    ...
                    # 여러 개 선택 시 Linux가 포함된 경우 경고
                    print(f"⚠️ Linux 인스턴스(번호: {choice_idx})는 단일 선택으로만 접속 가능합니다.")
...
# ----------------------------------------------------------------------------
# ElastiCache 접속 (v4.22 기반으로 새로고침 기능 추가)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.30')
