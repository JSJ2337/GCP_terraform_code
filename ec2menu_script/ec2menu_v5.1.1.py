...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache, ECS 접속 자동화 스크립트 v5.1.1 (디버깅 기능 추가)

v5.1.1 새로운 기능:
- 🔍 디버깅 기능: Role=jumphost 태그 문제 진단, SSM 상태 확인, 상세 로깅
- 🛠️ 문제 해결: 태그 없는 인스턴스에서도 점프호스트 선택 가능한 임시 옵션
...
class AWSManager:
    ...
    def list_ssm_managed(self, region: str, jump_host_tags: dict = None):
        ...
        try:
            ...
            # 디버깅: SSM describe_instance_information 상세 결과 확인
            print(colored_text(f"🔍 SSM describe_instance_information 호출 중...", Colors.DEBUG))
            response = ssm.describe_instance_information()
            info = response.get('InstanceInformationList', [])
            
            # 디버깅: SSM 응답 상세 정보
            print(colored_text(f"📊 SSM API 응답: {len(info)}개 인스턴스 정보 수신", Colors.DEBUG))
            
            # 페이지네이션 확인
            next_token = response.get('NextToken')
            if next_token:
                print(colored_text(f"⚠️  페이지네이션 토큰 발견! 더 많은 인스턴스가 있을 수 있습니다.", Colors.WARNING))
                print(colored_text(f"🔄 추가 페이지 조회 중...", Colors.INFO))
                
                while next_token:
                    response = ssm.describe_instance_information(NextToken=next_token)
                    additional_info = response.get('InstanceInformationList', [])
                    info.extend(additional_info)
                    next_token = response.get('NextToken')
                    print(colored_text(f"📄 추가 {len(additional_info)}개 인스턴스 정보 수신 (총 {len(info)}개)", Colors.DEBUG))
            
            # 디버깅: PingStatus별 통계
            status_counts = {}
            for instance in info:
                status = instance.get('PingStatus', 'Unknown')
                status_counts[status] = status_counts.get(status, 0) + 1
            
            print(colored_text(f"📈 SSM 인스턴스 상태별 통계:", Colors.DEBUG))
            for status, count in status_counts.items():
                color = Colors.SUCCESS if status == 'Online' else Colors.WARNING
                print(colored_text(f"   - {status}: {count}개", color))
            ...
            for res in resp.get('Reservations', []):
                for i in res.get('Instances', []):
                    ...
                    name = next((t['Value'] for t in i.get('Tags', []) if t['Key'] == 'Name'), f'unnamed-{instance_id}')
                    
                    # 태그 필터링 검사
                    if jump_host_tags:
                        instance_tags = {t['Key']: t['Value'] for t in i.get('Tags', [])}
                        
                        # 디버깅: 인스턴스의 모든 태그 확인 (처음 3개만 로깅)
                        if len(ssm_instances) < 3:
                            print(colored_text(f"🏷️  {name} ({instance_id}) 태그: {instance_tags}", Colors.DEBUG))
                    ...
                    ssm_instances.append({'Id': instance_id, 'Name': name})
            
            # 디버깅 정보 출력
            if jump_host_tags and filtered_count > 0:
                print(colored_text(f"🔍 {filtered_count}개 인스턴스가 태그 필터({jump_host_tags})로 제외되었습니다.", Colors.WARNING))
...
def choose_profile():
    ...
    while True:
        sel = input(colored_text("사용할 프로파일 번호 입력 (Enter=종료): ", Colors.PROMPT))
        if not sel:
            sys.exit(0)
...
def choose_jump_host(manager, region):
    ...
    # 디버깅을 위해 먼저 전체 EC2와 SSM 인스턴스 수를 비교
    print(colored_text(f"🔍 리전 {region}에서 인스턴스를 검색 중...", Colors.INFO))
    
    # 전체 EC2 인스턴스 수 확인
    all_ec2_instances = manager.list_instances(region)
    print(colored_text(f"🖥️  전체 EC2 인스턴스: {len(all_ec2_instances)}개", Colors.INFO))
    
    # SSM 관리 인스턴스 수 확인
    all_ssm_targets = manager.list_ssm_managed(region, None)  # 태그 필터 없이
    print(colored_text(f"📋 SSM 관리 인스턴스: {len(all_ssm_targets)}개", Colors.INFO))
    
    if len(all_ec2_instances) > len(all_ssm_targets):
        missing_count = len(all_ec2_instances) - len(all_ssm_targets)
        print(colored_text(f"⚠️  {missing_count}개 인스턴스가 SSM에서 누락되었습니다.", Colors.WARNING))
        
        # 누락된 인스턴스 상세 정보
        ssm_instance_ids = {target['Id'] for target in all_ssm_targets}
        missing_instances = []
        
        for ec2_instance in all_ec2_instances:
            if ec2_instance['InstanceId'] not in ssm_instance_ids:
                name = next((t['Value'] for t in ec2_instance.get('Tags', []) if t['Key'] == 'Name'), f"unnamed-{ec2_instance['InstanceId']}")
                missing_instances.append({
                    'Id': ec2_instance['InstanceId'],
                    'Name': name,
                    'State': ec2_instance['State']['Name'],
                    'Platform': ec2_instance.get('PlatformDetails', 'Unknown')
                })
        
        print(colored_text(f"🔍 누락된 인스턴스들 (처음 5개):", Colors.DEBUG))
        for instance in missing_instances[:5]:
            print(f"   - {instance['Name']} ({instance['Id']}) - {instance['State']}, {instance['Platform']}")
        
        if len(missing_instances) > 5:
            print(f"   ... 및 {len(missing_instances) - 5}개 더")
            
        print(colored_text("💡 가능한 원인:", Colors.DEBUG))
        print("   - SSM 에이전트 미설치/오래된 버전")
        print("   - IAM 역할 없음 (AmazonSSMManagedInstanceCore 필요)")
        print("   - 인스턴스가 stopped 상태")
        print("   - SSM 엔드포인트 연결 불가 (VPC/보안그룹)")
        print("   - 다른 AWS 계정의 인스턴스")
    
    if all_ssm_targets:
        print(colored_text("💡 발견된 SSM 인스턴스들:", Colors.INFO))
        for target in all_ssm_targets[:5]:  # 최대 5개만 표시
            print(f"   - {target['Name']} ({target['Id']})")
        if len(all_ssm_targets) > 5:
            print(f"   ... 및 {len(all_ssm_targets) - 5}개 더")
    ...
    if not ssm_targets:
        ...
        print(colored_text("💡 해결 방법:", Colors.INFO))
        print("   1. AWS 콘솔에서 EC2 > 인스턴스로 이동")
        print("   2. 점프호스트로 사용할 인스턴스 선택")
        print("   3. 태그 탭에서 '태그 관리' 클릭")
        print("   4. 새 태그 추가: 키='Role', 값='jumphost'")
        
        # 임시 해결책: 모든 SSM 인스턴스 중에서 선택 옵션 제공
        if all_ssm_targets:
            print(colored_text("\n🔧 임시 해결책: 사용 가능한 모든 SSM 인스턴스 중에서 선택하시겠습니까? (y/n): ", Colors.INFO), end="")
            choice = input().strip().lower()
            if choice == 'y':
                print(colored_text("\n--- [ 모든 SSM 인스턴스 ] ---", Colors.HEADER))
                for i, target in enumerate(all_ssm_targets, 1):
                    print(f" {i:2d}) {target['Name']} ({target['Id']})")
                print("--------------------------------\n")
                
                while True:
                    sel = input(colored_text("사용할 Jump Host 번호 입력 (b=취소): ", Colors.PROMPT)).strip().lower()
                    if sel == 'b':
                        return None
                    if sel.isdigit() and 1 <= int(sel) <= len(all_ssm_targets):
                        selected = all_ssm_targets[int(sel) - 1]
                        print(colored_text(f"✅ {selected['Name']} ({selected['Id']})를 점프호스트로 사용합니다.", Colors.SUCCESS))
                        return selected['Id']
                    print(colored_text("❌ 올바른 번호를 입력하세요.", Colors.ERROR))
    ...
    while True:
        sel = input(colored_text("사용할 Jump Host 번호 입력: ", Colors.PROMPT))
...
def ec2_menu(manager: AWSManager, region: str):
    ...
    try:
        while True:
            ...
            sel = input(colored_text("\n접속할 인스턴스 번호 입력 (r=새로고침, b=메인, n/t/s=정렬, batch=배치작업, 예: 1,2,3): ", Colors.PROMPT)).strip().lower()
            ...
            if not sel or sel == 'b':
                ...
                batch_sel = input("배치 작업할 인스턴스 번호들 입력 (예: 1,2,3,5): ").strip()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache/ECS 연결 도구 v5.1.1')
