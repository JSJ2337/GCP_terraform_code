...
# --- [HeidiSQL 연동] ---
# 1. HeidiSQL 설치 후 heidisql.exe 파일의 전체 경로를 여기에 입력하세요.
...
def connect_to_rds(manager: AWSManager, tool_path: str, region: str):
    """
    HeidiSQL의 안정적인 CLI를 사용하여 여러 DB(MSSQL 포함)에 동시에 자동으로 연결합니다.
    """
    ...
    sel = input("접속할 DB 번호 입력 (b=뒤로, 예: 1,2,3): ").strip().lower()
    if not sel or sel == 'b':
        return
    ...
    print("\nℹ️ 데이터베이스에 연결할 사용자 정보를 입력하세요. (모든 DB에 동일하게 적용됩니다)")
    try:
    ...
    try:
        ...
        if tool_path and Path(tool_path).exists():
            for i, choice_idx in enumerate(valid_choices):
                ...
                # --- [핵심 수정] ---
                # MSSQL 접속을 위한 네트워크 타입을 mssqloledb 로 변경
                network_type_map = {
                    'postgres': 'postgresql',
                    'mysql': 'mysql',
                    'mariadb': 'mariadb',
                    'sqlserver': 'mssqloledb', # mssql -> mssqloledb 로 수정
                }
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v5.0 (HeidiSQL 최종판)')
