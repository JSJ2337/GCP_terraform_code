...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v5.6 (로깅 오류 수정)

- v4.22 안정화 버전을 기반으로 기능 추가
- 각 메뉴에 목록 새로고침(r) 기능 추가
- Windows RDP 다중 접속 기능 추가 (쉼표로 구분하여 입력)
- RDP 접속 프로세스를 안정적으로 관리하도록 로직 개선
- Boto3 라이브러리와의 호환성을 위해 로깅 포맷 방식 수정
"""
import os
import sys
import argparse
...
def setup_logger(debug: bool):
    ...
    logging.basicConfig(level=level, format=fmt, style='{', handlers=handlers)
        ...
        try:
            ...
            for c in clus:
                ...
                result.append({
                    'Id':      c['CacheClusterId'],
                    'Engine':  c['Engine'],
                    'Address': ep.get('Address','') if ep else '',
                    'Port':    ep.get('Port',0) if ep else 0
                })
            return result
        except ClientError as e:
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v5.6')
