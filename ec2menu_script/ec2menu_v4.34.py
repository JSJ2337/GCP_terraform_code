...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v4.34 (SSM 파라미터 오류 수정)

- v4.33 버전을 기반으로 Linux SSM 접속 시 발생하던 파라미터 오류를 수정한 최종 안정화 버전.
- ssm_cmd 함수의 params 기본값을 올바른 JSON 형식으로 수정하여 `Unknown options` 오류를 해결.
- 다른 모든 기능은 v4.33과 동일하게 유지.
"""
import os
import sys
import argparse
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.34')
