...
# ----------------------------------------------------------------------------
# RDS 접속 (DBeaver 연결 창 호출 버전)
# ----------------------------------------------------------------------------
def connect_to_rds(manager: AWSManager, db_path: str, region: str):
    """
    터미널에서 직접 DB 사용자 이름과 비밀번호를 입력받아
    DBeaver로 데이터베이스에 바로 연결합니다.
    """
    ...
    sel = input("접속할 DB 번호 입력 (b=뒤로): ").strip().lower()
    if not sel or sel == 'b':
        return

    if not sel.isdigit() or not (1 <= int(sel) <= len(dbs)):
        print("❌ 유효한 번호를 입력하세요.")
        return
    ...
    try:
        ...
        print("\n✅ 포트 포워딩 활성화. DBeaver에 직접 연결합니다...")

        if db_path and Path(db_path).exists():
            driver_map = {
                'postgres': 'postgresql', 'aurora-postgresql': 'postgresql',
                'mysql': 'mysql8', 'aurora-mysql': 'mysql8', 'mariadb': 'mariadb',
            }
            driver = next((v for k, v in driver_map.items() if k in db['Engine']), db['Engine'])

            # 입력받은 사용자 정보로 DBeaver 연결 문자열 생성
            con_string = f"driver={driver}|host=localhost|port={local_port}|user={db_user}|password=\"{db_password}\""

            subprocess.Popen([db_path, "-con", con_string], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def connect_to_cache(manager: AWSManager, region: str):
    """ElastiCache 클러스터에 접속합니다. 접속 종료 후 다시 목록을 표시합니다."""
    while True:
        ...
        if not sel.isdigit() or not (1 <= int(sel) <= len(clus)):
            print("❌ 유효한 번호를 입력하세요.")
            time.sleep(1)
            continue # 다시 선택하도록 루프의 처음으로 돌아감
