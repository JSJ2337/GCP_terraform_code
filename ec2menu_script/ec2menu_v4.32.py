...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v4.32 (f-string 문법 오류 수정)

- v4.31 버전을 기반으로 f-string 문법 오류를 수정한 안정화 버전.
- RDS/ElastiCache 접속 시 포트 포워딩 파라미터 생성 부분의 문법 오류를 해결.
- 다른 모든 기능은 v4.31과 동일하게 유지.
"""
import os
import sys
import argparse
    ...
    while True:
        ...
        try:
            for i, choice_idx in enumerate(valid_choices):
                ...
                params = f'{{ "host": ["{\n"db["Endpoint"]}"], "portNumber": ["{\n"db["Port"]}"], "localPortNumber": ["{\n"local_port}"] }}'
...
def connect_to_cache(manager: AWSManager, region: str):
    while True:
        ...
        params = f'{{ "host": ["{\n"c["Address"]}"], "portNumber": ["{\n"c["Port"]}"], "localPortNumber": ["{\n"local_port}"] }}'
        proc = None
        try:
            proc = subprocess.Popen(
                ssm_cmd(manager.profile, region, target=tgt, doc='AWS-StartPortForwardingSessionToRemoteHost', params=params),
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.32')
