...
def launch_db_tool(path, port):
    ...
    subprocess.Popen([path,'--connect',f'localhost:{port}'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, stdin=subprocess.DEVNULL, close_fds=True)
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            sel = input("선택 (b=뒤로 /cancel=Enter): ").strip().lower()
            ...
            if sel == '1':
                ...
                insts = list_instances(ec2_client)
                if not insts:
                    print("⚠ 실행 중인 인스턴스 없음")
                    continue
                while True:
                ...
                while True:
                    ...
                    sel_c = choose_cache_cluster(clusters)
                    if not sel_c:
                        break  # 클러스터 메뉴 빠져나와 서비스 메뉴로
                    local = base_port + 100
                    print(f"🔹 [{sel_c['Id']}] {sel_c['Address']}:{sel_c['Port']} -> localhost:{local}")
                    proc = start_cache_port_forward(profile, region, sel_c['Address'], sel_c['Port'], local)
                    for _ in range(10):
                        ...
                        proc.terminate()
                        continue
                    launch_cache_cli(sel_c['Engine'], local)
                    proc.terminate()
                continue  # 반복 종료 시 서비스 메뉴로 복귀

            else:
                print("❌ 올바른 메뉴 선택")

        # end Service 메뉴 loop – 다시 Region 선택으로
