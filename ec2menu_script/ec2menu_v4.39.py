...
# -*- coding: utf-8 -*-
"""
EC2, RDS, ElastiCache 접속 자동화 스크립트 v4.39 (HeidiSQL 세션 이름 표시 수정)

- RDS 접속 시 HeidiSQL 세션 목록에 'LOCALHOST' 대신 RDS 인스턴스 ID가 표시되도록 수정.
- `connect_to_rds` 함수에서 HeidiSQL 실행 시 `-session` 대신 `--description` 파라미터를 사용하도록 수정.
"""
import os
import sys
import argparse
...
# ----------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description='AWS EC2/RDS/ElastiCache 연결 도구 v4.39')
