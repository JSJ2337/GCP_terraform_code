...
- 모든 terragrunt.hcl 파일 (00-project ~ 12-dns)
- 20-storage/main.tf
...
## 2025-11-25 추가 작업 (Session 2) - Bootstrap 4레이어 모듈화 및 delabsgames.gg 배포

### 1. Bootstrap 4레이어 구조로 리팩토링

**목적**: 기존 단일 main.tf → 4개의 독립 레이어로 분리하여 관리성 향상

| 레이어 | 역할 | 주요 리소스 |
|--------|------|-------------|
| `00-foundation` | 기본 인프라 | Service Account, API 활성화 |
| `10-network` | 네트워크 | VPC, Subnet, Router, NAT, Firewall |
| `20-storage` | 스토리지 | GCS Bucket (Terraform State) |
| `50-compute` | 컴퓨팅 | Jenkins VM, Bastion VM |

#### 파일 구조

```
bootstrap/
├── root.hcl              # 공통 Terragrunt 설정
├── common.hcl            # 공통 변수 (organization_id, project_id 등)
...
### 2. delabsgames.gg 조직에 Bootstrap 배포

**대상 조직**: delabsgames.gg (Organization ID: 1034166519592)

**관리 프로젝트**: delabs-gcp-mgmt

#### 배포된 리소스

| 리소스 | 이름 | 상태 |
|--------|------|------|
| Service Account | jenkins-terraform-admin@delabs-gcp-mgmt.iam.gserviceaccount.com | ✅ |
| VPC | delabs-gcp-mgmt-vpc | ✅ |
| Subnet | delabs-gcp-mgmt-subnet (10.100.0.0/24) | ✅ |
| Cloud Router | delabs-gcp-mgmt-router | ✅ |
| Cloud NAT | delabs-gcp-mgmt-nat | ✅ |
| Firewall | delabs-gcp-mgmt-vpc-allow-iap-ssh | ✅ |
| Firewall | delabs-gcp-mgmt-vpc-allow-jenkins | ✅ |
| Firewall | delabs-gcp-mgmt-vpc-allow-internal | ✅ |
| GCS Bucket | delabs-terraform-state-live | ✅ |
| VM | delabs-terraform-jenkins | ✅ |
| VM | delabs-bastion | ✅ |

---

### 3. Terragrunt Dependency 에러 해결

**증상**: `Unknown variable; There is no variable named "dependency"`

**원인**: Local backend 사용 시 dependency output 참조 불가

**해결**:

```hcl
# 각 레이어의 terragrunt.hcl
dependency "foundation" {
  config_path  = "../00-foundation"
  skip_outputs = true  # 출력 참조 비활성화
}

# common.hcl에서 직접 값 참조
inputs = merge(
...
### 4. coalesce 함수 에러 해결 (50-compute)

**증상**: `Invalid value for "vals" parameter: cannot search an empty list`

**원인**: `coalesce()` 함수에 null/빈 값 전달
...
**변경 전**: `delabs-gcp-mgmt-mgmt-vpc` (mgmt 중복)

**변경 후**: `delabs-gcp-mgmt-vpc`

```hcl
# 10-network/main.tf
locals {
  network_name = "${var.management_project_id}-vpc"      # 변경
  subnet_name  = "${var.management_project_id}-subnet"   # 변경
}
```

---

### 6. Terraform State를 GCS로 마이그레이션

**목적**: Local backend → GCS backend로 State 마이그레이션

**방법**: Jenkins `import-state` 액션 사용

#### GCS State 경로

```
gs://delabs-terraform-state-live/bootstrap/
├── 00-foundation/default.tfstate (17KB)
├── 10-network/default.tfstate (15KB)
├── 20-storage/default.tfstate (5KB)
└── 50-compute/default.tfstate (32KB)
```

#### import-state 액션 추가 (Jenkinsfile)
...
stage('Import State') {
    ...
    steps {
        script {
            // 각 레이어별 terraform import 명령 실행
            // 00-foundation: Service Account
            // 10-network: VPC, Subnet, Router, NAT, Firewall
            // 20-storage: GCS Bucket, IAM
            // 50-compute: Boot Disk, VM Instance
        }
    }
}
```
...
| 커밋 | 설명 |
|------|------|
| `c6c8a8c` | refactor(bootstrap): 4개 레이어로 모듈화 (foundation, network, storage, compute) |
| `54a3895` | fix(bootstrap): dependency output 에러 해결 - skip_outputs 적용 |
| `2dcb4b8` | fix(50-compute): coalesce 함수 에러 수정 |
| `bd6a195` | fix(common): VPC/Subnet 이름 수정 (실제 리소스명 반영) |
| `f52ebbe` | refactor(10-network): 네트워크 리소스 네이밍 간소화 |
| `0a5bec7` | feat(jenkins): import-state 액션 추가 |
| `e7c3f71` | Revert "feat(jenkins): migrate-state 액션 추가" |
...
- [x] 4개 레이어 모두 GCP에 배포 완료
- [x] GCS State 마이그레이션 완료
- [x] Jenkins import-state 액션 정상 동작
- [x] `plan` 실행 시 "No changes" 확인 필요

---

### 다음 단계

- [ ] Jenkins VM에 Docker/Jenkins 설치 확인
- [ ] `plan` 실행하여 "No changes" 확인
- [ ] Bootstrap 파이프라인 운영 안정화
