...
5. **로드 밸런서 자동 백엔드 연동 복구**
   - Terragrunt가 `50-workloads` 출력(Instance Group self link)을 읽지 못해 신규 VM이 LB 백엔드에 자동 등록되지 않던 문제를 해결
   - `proj-default-templet` 및 jsj-game-{k,l} 환경의 `70-loadbalancer/terragrunt.hcl`에 `dependency "workloads"` 블록을 추가해 `auto_instance_groups` 입력이 실제 값으로 채워지도록 수정하고 plan/validate용 mock output 정의
...
- 새로 생성한 VM Instance Group이 자동으로 Load Balancer 백엔드에 추가되지 않던 문제 → Terragrunt dependency 출력 연결 복구
