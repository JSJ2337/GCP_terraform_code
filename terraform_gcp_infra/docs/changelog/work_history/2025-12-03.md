...
## 9. Bootstrap Terragrunt 에러 수정

### 배경
- Section 8에서 Bootstrap 일반화 작업 후 Jenkins 배포 중 에러 발생
- `bootstrap/10-network/terragrunt.hcl` 리팩토링 과정에서 Terragrunt 제약사항 위반
...
multiple locals blocks in a single config. Consolidate to a
single locals block.
```

#### 원인
- `terragrunt.hcl` 파일 내에 두 개의 locals 블록 존재
  - Line 9: 첫 번째 locals 블록 (parent_dir, common_vars, layer_vars)
  - Line 77: 두 번째 locals 블록 (projects, psc_endpoints_gcby, etc.)
- Terragrunt는 **단일 locals 블록만** 허용
...
**커밋 메시지**: "fix: bootstrap/10-network terragrunt.hcl - 중복 locals 블록 통합"

---

### 에러 2: Dependency Reference in Locals
...
#### 원인
- locals 블록 내에서 `dependency.*` 참조 시도
- **Terragrunt 제약사항**: locals 블록에서는 오직 다른 local 변수만 참조 가능
- dependency는 locals 외부에서 정의되므로 직접 참조 불가
...
#### 해결 (Commit: cd7d525)
PSC endpoints 생성 로직을 **inputs 블록으로 이동** (inputs에서는 dependency 참조 가능)
...
locals {
  psc_endpoints_gcby = {
    "gcby-cloudsql" = {
      target_service_attachment = dependency.gcby_database.outputs...  # ❌
    }
  }
  all_psc_endpoints = merge(local.psc_endpoints_gcby)
}
...
**커밋 메시지**: "fix: bootstrap/10-network - PSC endpoints를 inputs 블록으로 이동 (locals에서 dependency 참조 불가)"
...
| 블록 타입 | 참조 가능 대상 | 예시 |
|----------|--------------|------|
| **locals** | - 다른 local 변수만<br>- read_terragrunt_config()<br>- Terraform 내장 함수 | `local.parent_dir`<br>`local.common_vars.locals.projects` |
| **inputs** | - local 변수<br>- dependency outputs<br>- 모든 Terragrunt 함수 | `local.projects`<br>`dependency.gcby_database.outputs...` |
| **dependency** | - local 변수 (config_path에서) | `local.common_vars.locals.projects.gcby.database_path` |
...
**Commit: d30857e**
```
fix: bootstrap/10-network terragrunt.hcl - 중복 locals 블록 통합

Terragrunt는 하나의 locals 블록만 허용
- 두 개의 locals 블록을 하나로 통합
- 변수 순서 정리 (기본 변수 → 파생 변수)
```

**Commit: cd7d525**
```
fix: bootstrap/10-network - PSC endpoints를 inputs 블록으로 이동 (locals에서 dependency 참조 불가)

Terragrunt 제약사항:
- locals 블록 내에서는 다른 local 변수만 참조 가능
- dependency 참조는 inputs 블록에서만 가능

변경사항:
- locals: dependency 참조하던 psc_endpoints_gcby, all_psc_endpoints 제거
- inputs: PSC endpoints 동적 생성 로직 이동
- 기능은 동일하게 유지, 위치만 변경
```
...
**bootstrap/10-network/terragrunt.hcl**
- locals 블록 통합 (Line 12-30)
- PSC endpoints 생성 로직을 inputs로 이동 (Line 87-132)
- 총 변경: +38줄, -46줄
...
1. **locals 블록의 제약**
   - 파일당 하나만 허용
   - 외부 리소스(dependency) 참조 불가
   - 순수 데이터 변환/가공만 수행

2. **inputs 블록의 역할**
   - Terraform 변수로 전달할 최종 값
   - dependency outputs 참조 가능
   - 복잡한 동적 생성 로직 수행 가능

3. **의존성 평가 순서**
   ```
   1. dependency 블록 평가
   2. locals 블록 평가 (local 변수만 사용)
   3. inputs 블록 평가 (local + dependency 모두 사용)
   ```

#### 리팩토링 시 주의사항
- Terragrunt 제약사항을 먼저 확인
- 블록 간 참조 규칙 이해 필요
- Jenkins CI/CD로 조기 검증
...
#### 즉시 수행
- ✅ Jenkins bootstrap 재실행으로 검증

#### 향후 개선
1. **새 프로젝트 추가 문서화** (우선순위: 높)
   - Terragrunt 제약사항 명시
   - inputs 블록에서 PSC endpoints 추가 방법 안내

2. **리팩토링 체크리스트** (우선순위: 중)
   - Terragrunt 블록별 제약사항 정리
   - CI/CD 파이프라인 강화
