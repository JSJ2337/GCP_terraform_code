...
locals {
  ...
  remote_state_bucket   = get_env("TG_STATE_BUCKET", "REPLACE_REMOTE_STATE_BUCKET")
  remote_state_project  = get_env("TG_STATE_PROJECT", try(local.common_inputs.management_project_id, "REPLACE_MANAGEMENT_PROJECT_ID"))
  remote_state_location = get_env("TG_STATE_LOCATION", "REPLACE_REMOTE_STATE_LOCATION")
...
inputs = {
  ...
  # ⚠️ 아래 기본값은 create_project.sh에서 자동 치환됨
  org_id          = get_env("TG_ORG_ID", "REPLACE_ORG_ID")
  billing_account = get_env("TG_BILLING_ACCOUNT", "REPLACE_BILLING_ACCOUNT")
