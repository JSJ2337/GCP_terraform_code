# 65-cache 레이어
> Terragrunt: environments/LIVE/jsj-game-m/65-cache/terragrunt.hcl


Google Cloud Memorystore for Redis(Standard HA 기본) 인스턴스를 배포합니다. 10-network 레이어가 생성한 전용 VPC에 프라이빗으로 연결하며, naming 모듈 규칙을 따라 일관된 리소스 이름과 라벨을 부여합니다. Enterprise/Enterprise Plus 티어를 선택하면 읽기 엔드포인트와 샤딩 옵션까지 구성할 수 있습니다.

## 주요 기능
- Standard HA Redis 인스턴스 생성 (Primary + Replica 자동 구성)
- Enterprise 티어 선택 시 Replica Count / Read Endpoint / Shard 옵션 지원
- VPC Direct Peering 또는 PSC 기반 프라이빗 접속
...
2. 필수 수정 항목:
   ...
   - Enterprise tier 사용 시 `tier`, `replica_count`, `shard_count`, `connect_mode`(PSC) 등을 조정
