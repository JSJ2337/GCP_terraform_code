# 10-network 레이어
> Terragrunt: environments/LIVE/jsj-game-m/10-network/terragrunt.hcl
...
```hcl
additional_subnets = [
  { name = "game-m-subnet-dmz",     region = "asia-northeast3", cidr = "10.3.0.0/24" },
  { name = "game-m-subnet-private", region = "asia-northeast3", cidr = "10.3.1.0/24" },
  { name = "game-m-subnet-db",      region = "asia-northeast3", cidr = "10.3.2.0/24" }
]
...
- 서브넷 self-link는 `additional_subnets`에서 선언한 값을 그대로 사용해야 하므로 50-workloads/70-loadbalancers/* 등에서도 명시적으로 입력해야 합니다.
