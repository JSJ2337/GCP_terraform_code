# 70-loadbalancers/lobby 레이어
> Terragrunt: environments/LIVE/jsj-game-m/70-loadbalancers/lobby/terragrunt.hcl


HTTP(S) 및 내부 로드밸런서를 비롯해 다양한 GCP Load Balancer 구성을 담당합니다. 백엔드 인스턴스 그룹을 연결하고 헬스 체크, CDN, IAP, SSL 등을 설정할 수 있습니다.  
이 레이어는 `jsj-lobby-*` 이름의 Instance Group만 자동으로 `auto_instance_groups`에 주입하도록 구성되어 있습니다.
...
2. 주요 항목 설명:
   ...
   - 내부 LB일 경우 `region`, `network`, `forwarding_rule_ports`, `internal_subnetwork_self_link`(예: game-m-subnet-private self-link) 지정

## Terragrunt 실행
```bash
cd environments/LIVE/jsj-game-m/70-loadbalancers/lobby
...
- 내부 LB는 네트워크와 서브넷을 명시적으로 지정해야 하며, 백엔드 인스턴스 그룹이 해당 서브넷에 존재해야 합니다. DMZ와 분리된 Private 서브넷 self-link를 사용하면 내부 트래픽만 수용하는 Load Balancer를 구성할 수 있습니다.
