# 70-loadbalancers/web 레이어
> Terragrunt: environments/LIVE/jsj-game-m/70-loadbalancers/web/terragrunt.hcl


HTTP(S) 및 내부 로드밸런서를 비롯해 다양한 GCP Load Balancer 구성을 담당합니다. 백엔드 인스턴스 그룹을 연결하고 헬스 체크, CDN, IAP, SSL 등을 설정할 수 있습니다.  
`jsj-web-*` Instance Group만 자동으로 백엔드에 연결되도록 `auto_instance_groups` 필터가 적용되어 있습니다.
...
## Terragrunt 실행
```bash
cd environments/LIVE/jsj-game-m/70-loadbalancers/web
