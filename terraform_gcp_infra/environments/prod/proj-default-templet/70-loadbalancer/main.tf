...
locals {
  ...
  # Default names
  default_backend_service_name = "${local.project_prefix}-backend"
  default_forwarding_rule_name = "${local.project_prefix}-lb"
  default_health_check_name    = "${local.project_prefix}-health"
}
...
module "load_balancer" {
  ...
  # Backend Service
  backend_service_name = var.backend_service_name != "" ? var.backend_service_name : local.default_backend_service_name
  ...
  health_check_name                  = var.health_check_name != "" ? var.health_check_name : local.default_health_check_name
  ...
  # Forwarding Rule
  forwarding_rule_name      = var.forwarding_rule_name != "" ? var.forwarding_rule_name : local.default_forwarding_rule_name
